...
description: 从一本真实书籍出发，用陌言样例的情绪文案结构和“十二”样例的固定毛笔字遮罩、9 张真实书封轮播、书名与目标封面同页锁定、人物侧脸加明亮风景的动态长镜头，生成音画严格同长的情绪读书视频。第一阶段同时锁定主题、逐字稿和只能由两个汉字组成的毛笔遮罩，并生成遮罩确认图供用户审核；确认后后期必须复用同一字形。书名口播固定拆成“今天分享的是”与书名两个节拍，书名前后保留可验证停顿。后期使用事件驱动依赖图自动并行生产，任一素材满足依赖后立即推动对应镜头，不等待同批其他素材。静态图片默认使用交替 zoom in、zoom out、pan left、pan right 和 emotional hold 的确定性轻运动，并按语义选择硬切或短淡化。默认一次性交付 1080×1440 高清 H.264/AAC 成片和基于同一时间轴的可编辑剪映草稿。用户要求制作、复刻、加速、并行生成、继续或修改情绪读书视频，要求第一阶段确认遮罩、两字遮罩、书名停顿与重音、图片动态变化、文案对应生图、人物侧脸风景构图、书封轮播、书名封面同页展示、1080p 高清、剪映草稿或双交付时使用。
---
...
- 正文生图默认使用 Codex App 内置 `image_gen`，提供方记为 `codex_imagegen`；用户明确指定时才可切换 `grok_local` 或 `apimart`。APIMart 涉及按量费用，禁止作为自动回退。正文轻动态视频默认使用 `grok-local/grok_generate_video` 的 `source_image` 模式。把实际选择写入 `manifest.defaults.image_generation`、`manifest.defaults.video_generation` 和 `07-分镜/image-generation-vNNN.json`，禁止静默换渠道。任何密钥或 token 不写入 Skill、项目文件或日志。
...
- 正文生图必须采用 `parallel_individual + parallel_all`：先冻结全部独立分镜提示词和输出路径，再把全部 `shot-NN.png` 同批提交，所有请求提交完成后才统一等待。片头遮罩背景不属于生图批次。每个请求直接生成一张最终独立图片；禁止拼图、母版、网格和二次裁切，禁止先生成 `visual-lock` 再逐张生成，也禁止在循环里逐项 `await`。全部结果统一质检，失败图片只允许组成一个并发补图批次。每张会进入成片的无文字正文图仍必须以自身作为 `source_image` 转成 Grok 轻动态片段。真实书封、轮播卡、毛笔字遮罩、字幕图和其他含文字素材不送入模型动态化。单张视频生成失败时最多收紧提示词重试 1 次，仍失败才记录 `ffmpeg_fallback` 并使用连续微动，不得静默跳过。Remotion 不参与生视频或镜头动画。
...
3. 把逐字稿划分为钩子、遮罩展开、9 张轮播、锁书、处境、转折、释放和末句八个声音环节。
...
使用已锁定配音制作完整声音预览。纯音乐在说话时自动闪避 3–5dB，旁白主观领先 10–14dB。BGM 必须清楚提供情绪底色，不能只在耳机里勉强听见；旁白段的乐器底床从 -26至-23 LUFS 起试。带演唱的曲库歌曲必须拆成“乐器底床＋演唱层”思路处理：保留音乐情绪，让演唱比乐器再低 10–16dB，旁白期间对演唱层额外闪避 4–8dB。歌词不得连续可辨，也不得压低整条 BGM 到失去情绪。
...
按 [grok-oauth-video-spec.md](references/grok-oauth-video-spec.md) 调用 `grok_status` 做授权和能力预检，不得读取或落盘 token。把本关卡内部验收通过且会进入成片的全部 `shot-NN.png` 加入图生视频计划，不再人工挑少数镜头。
...
5. 运行 `build_book_carousel_cards.py`，从 Skill 的真实书封库生成项目独立的 9 张全屏卡，并排除目标书。
6. 每张轮播卡必须做 3–4 帧的 `snap_settle` 动效：入场首帧 103%–104% 轻微放大并带 1–2px 柔焦，随后在本卡剩余帧内快速收稳到 100%；卡与卡之间仍是硬切，不滑动、不叠化、不弹跳。九次短促变化形成“咔咔咔后停住”的视觉节奏，不能把静态 PNG 直接连续拼接。
7. 片头使用 3 帧目标画面预闪，再按实测时间轴完成遮罩、动态轮播和 `cover_waterwave_then_title_drop` 锁定：旁白先完整说“今天分享的是”，停 0.45–0.65 秒后，齿轮声结束并进入目标真实书封时单独重读书名；书名念完再停 0.45–0.70 秒才进入正文。目标书封必须独立展示 0.65–0.85 秒。书名从目标封面出现的第 1 帧起就必须叠在该封面页上方安全区，在封面始终可见的前提下用 10–14 帧 ease-out 由大到小收稳。禁止“先只有封面，切到另一页再出书名”。
8. 先运行 `build_target_cover_title_page.py` 生成封面基底页和书名封面同页定帧。水滴声触发时，必须以“书名＋真实封面＋背景”的同页定帧为输入，对整页做连续位移映射；书名、封面文字、边缘和背景必须同步低幅折射，0.30–0.50 秒内起伏并连续恢复。水波素材必须是独立的整帧不透明 MP4，不叠加圆环、可见水珠、落点飞溅、同心圆或高光圈。水波结束后切入目标主画面时，书名保持在同一顶部位置，不重新入场、不跳位，作者再淡入。手机预览中看不出“书名和封面同页、整页一起水波形变”的一律判失败。
9. 生成 `opening-vNNN.json` 并逐帧检查 0–6 秒。

片头声音不得重新设计，必须引用 `music-cue-vNNN.json` 的钩子、遮罩、9 张轮播、锁书配置。将 `mask-source-vNNN.mp4`、`opening-mask-plan-vNNN.json`、`masked-keyword-expand-vNNN.mp4` 和 `opening-vNNN.json` 全部登记到 `manifest.steps.visual_package.artifacts`，剪映草稿也必须将遮罩源视频保留在 `Resources/local_media/`。
...
- 目标书锁定阶段必须在真实封面所在的同一页上显示书名；书名在封面上方安全区由大到小收稳，封面全程保持可见，作者随后出现。书名落位后与封面一起接受整页水波形变，切入正文时保持同一顶部位置到结尾。
