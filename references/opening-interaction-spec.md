...
| 0.10–1.35s | `hook_keyword` | 黑底；毛笔关键词内部播放同源桥段视频 | 上方两行情绪句完整显示 |
| 1.35–1.45s | `horizontal_cut` | 从关键词垂直中心出现横线并延长至全宽 | 情绪句和关键词保持 |
| 1.45–2.90s | `mask_expand` | 同一桥段视频从横带向上、向下展开至全屏 | 文字被展开画面逐步覆盖 |
| 2.90–4.10s | `book_carousel` | 9 张非目标真实书封卡，每张 3–4 帧内由 103%–104% 快速收稳至 100%，卡间硬切 | 卡片不叠加新文字 |
| 4.10–4.85s | `target_cover_hold` | 齿轮声结束后展示目标真实书封；封面始终保持可见；4.45s 起书名、封面和背景一起做整帧水波位移并恢复到 4.85s | 书名从第 1 帧起就位于封面上方同页安全区，10–14 帧内由大到小收稳 |
| 4.85–5.55s | `target_lock` | 水纹结束后硬切目标主画面 | 书名保持与封面页落位时相同的顶部位置，不重新入场；作者再淡入 |
| 约 5.55s 起 | `body_enter` | 主画面继续或 0.3–0.5s 叠化到第一正文镜头 | 第一张正文字幕进入 |
...
- 轮播严格 9 张，在 4.10 秒结束；目标书封展示 0.65–0.85 秒，整页水面形变全部发生在封面阶段，4.75–4.95 秒切入主画面。
- 正文不早于 5.30 秒，默认为 5.55 秒。
...
4. `hook_text`：白色杨任东竹石体 Heavy 情绪句，在遮罩覆盖前保持稳定。
...
原始书封库在 `assets/book-card-library/`。每个项目用本地脚本生成独立版本卡片：

```bash
python3 <skill_root>/scripts/build_book_carousel_cards.py \
  --manifest <skill_root>/assets/book-card-library/manifest.json \
...
- 9 张 1080×1440 PNG 书封卡。
- 9 张均为 0.10–0.14 秒，整段约 1.15 秒。
- `carousel-cards-vNNN.json`，记录书名、作者、源书封、卡片文件和时长。
...
- 卡片之间全屏硬切，不滑动、不叠化、不弹跳，不为每张卡加 whoosh。禁止把 9 张静态 PNG 无动效直拼。
- 目标书严格从 9 张快速卡中排除。
...
- 书名落位后，水滴声主瞬态触发“书名＋封面＋背景”整页波浪位移，0.30–0.50 秒内连续起伏并恢复，再硬切目标动态主画面。书名在切换前后必须保持同一顶部位置和尺寸，不重新入场。
- 作者不与中央大书名同时抢画面；书名接近顶部后作者再淡入。书名与作者落位后保持到结尾，不随正文切镜重新入场。
- 锁定开始同步一次柔和水滴/水波尾韵音；目标书名朗读重音落在锁定开始后 0–0.20 秒。
- 目标书名念完后留 0.3–0.6 秒呼吸，正文默认在 5.55 秒进入；正文进入时书封展示页叠化到第一正文镜头，顶部书名作者保留。

### 水滴式书名锁定

2026-08-01 对 `modal_id=7668678283642779072` 的 30fps 原片重新逐帧测量：目标书封先独立出现，水滴音瞬态后进入目标主画面。2026-08-02 用户针对《自控力》片头再次明确：生产版不能把书名和封面拆成两个页面；书名必须在目标封面所在页的上方安全区出现并由大到小收稳，随后与封面一起发生整页水波折射。这条用户明确规则优先于早期原片观察。
...
- 水滴音主瞬态与 `wave_trigger_at` 使用同一时间点，最大误差 1 帧；`hero_cut_at` 必须晚于触发点 0.30–0.50 秒，并与水波素材结束对齐。主画面顶部书名的位置和尺寸必须与封面页落位帧一致。
...
- “今天分享的是”应在约 2.25–3.60 秒内完成，横跨遮罩展开末段和轮播初段。
- 引导句末到目标书名首字必须留 0.45–0.65 秒实测停顿。目标书名在约 4.00–5.25 秒单独朗读，使用低位、结实、下收的重音，并与目标封面同页书名的出现同步。
- 书名末字到正文首字必须再留 0.45–0.70 秒实测停顿。正文第一句在 5.30–5.85 秒之间开始，不在 4 秒左右提前 J-cut。
- 轮播结构音固定使用剪映原声 `assets/sfx-library/jianying-carousel-clockwork-v001.mp3`（“闹钟上发条旋钮转动齿轮”）。从轮播开始连续播放；跳过文件开头约 83ms 静音，在轮播结束处裁切并做 20–40ms 淡出，不变速、不拆成九个点击，也不为每张卡重复触发完整音效。
...
- 画面固定使用 `waterdrop_lock_response.type=cover_waterwave_then_title_drop`。`sfx_at` 与 `wave_trigger_at`、`visual_end` 与 `hero_cut_at`、`hero_cut_at` 与标题第 0 帧的误差分别不得超过 1 帧；缺少任一项均视为锁定失败。
...
`carousel_cards` 实际必须为 9 项，长度与 `card_durations` 一致，所有 `asset` 均存在，所有 `role` 均为 `carousel`，且不得与 `bookMeta.title` 相同。`keyword_mask_style` 只有在内容包确认图中查看蒙版边缘、字形和重心并确认后，才能从生成器的 `custom_review_required` 改成 `custom_reviewed`。`opening.keyword` 必须与最新 `mask-keyword-vNNN.json.keyword` 完全一致且恰好两个汉字，`keyword_mask_asset` 必须与其 `alpha_mask` 逐像素一致。
...
5. 轮播恰好 9 张真实书封卡，每张约 3–4 帧；逐张确认首帧轻放大/柔焦、末帧收稳，卡间全部硬切。
6. 目标书不出现在快速轮播，而是在约 4.1 秒独立锁定。
7. 目标真实书封清楚可见并展示 0.65–0.85 秒；书名从该封面页第 1 帧起就在封面上方安全区出现，10–14 帧内由大到小收稳。逐帧确认书名动画全程保留封面，没有切到另一页。
8. 书名落位后，逐帧确认书名、封面文字、封面边缘和背景同时低幅形变，峰值后连续恢复。不得出现可见水珠、下落轨迹、飞溅、椭圆/同心圆轮廓或高光环。水滴音与波浪触发误差不超过 1 帧，水波持续 0.30–0.50 秒后再切主画面；书名在切换前后保持相同顶部位置，不重新入场。
9. 书名重音与锁定点同步；正文不早于 5.3 秒，书名说完后有呼吸，音乐和旁白没有断点。
